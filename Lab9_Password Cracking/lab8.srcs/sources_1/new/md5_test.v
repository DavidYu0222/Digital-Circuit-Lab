...
//localparam [1:0] S_MAIN_INIT = 2'b00, S_MAIN_CALC = 2'b01, S_MAIN_COMP = 2'b10;
...
integer r_bar = 32;
/*
localparam [1:0] S_MAIN_A = 2'b00, S_MAIN_B = 2'b01,
                            S_MAIN_C = 2'b10;
reg[1:0] P, P_next;
*/
reg [0:63] w;
...
always @(*) begin // FSM next-state logic
  case (P)
    S_MAIN_A:
        if (enable) P_next <= S_MAIN_B;
        else P_next <= A;
    S_MAIN_B:
        if (valid) P_next <= S_MAIN_C;
        else P_next <= S_MAIN_B;
    S_MAIN_C:
        if (~enable) P_next <= S_MAIN_A;
